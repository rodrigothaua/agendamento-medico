    ...
    <x-card>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                @if($appointment->status !== 'completed')
                                    <select onchange="AppointmentManager.updateStatus({{ $appointment->id }}, this.value)" 
                                ...
                                <x-button 
                                    size="sm" 
                                    variant="outline"
                                    onclick="AdminUtils.redirect('{{ route('admin.appointments.show', $appointment->id) }}')"
                                >
                                    Ver
                                </x-button>
                                ...
                                <x-button 
                                    size="sm" 
                                    variant="danger"
                                    onclick="AppointmentManager.confirmDelete({{ $appointment->id }}, '{{ $appointment->patient->name }}', '{{ \Carbon\Carbon::parse($appointment->scheduled_at)->format('d/m/Y H:i') }}')"
                                >
        ...
        @else
            <div class="text-center py-12">
                ...
                <x-button 
                    variant="primary"
                    onclick="AdminUtils.redirect('{{ route('admin.appointments.create') }}')"
                >
